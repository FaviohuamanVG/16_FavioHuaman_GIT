    ...
    <div class="card" id="divDatosVenta">
        ...
        <div class="card-body">
            ...
            <form method="post" action="VtaGrabar" id="formVenta">
                ...
                <div class="form-group row">
                    <label for="vtaTipoEnvio" class="col-sm-2 col-form-label">Tipo de Envio</label>
                    <div class="col-sm-10">
...
<script>
    ...
    function fnBusCargarItem(){
        ...
        $.getJSON( url, function( data ) {
            ...
            $.each(data, function (key, prod) {
                ...
                html += "<td><button type='button' class='btn btn-danger' onclick='fnEliminarItem(" + key + ")'>Eliminar</button></td>"; // Añadir botón de eliminar
                html += "</tr>";
                total += prod.subtotal;
            });
            $("#vtaItems").html(html);
    ...
    function fnEliminarItem(index) {
        // Obtener la lista de items desde el HTML
        let listaItems = $("#vtaItems").find("tr");

        // Eliminar la fila correspondiente
        listaItems.eq(index).remove();

        // Recalcular el total
        let total = 0.0;
        listaItems.each(function () {
            total += parseFloat($(this).find("td").eq(4).text()); // Sumar los subtotales
        });
